...
const AdminStatsPage = () => {
  ...
  const handleAdd = () => {
    ...
    toast({
      title: 'Statistics added',
      description: `Player statistics have been recorded successfully.`,
      status: 'success',
      duration: 3000,
    });
    onAddClose();
  ...
  const handleDelete = () => {
    ...
    toast({
      ...
      description: `Statistics record has been removed.`,
      status: 'success',
      duration: 3000,
    });
    onDeleteClose();
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <VStack align="start" spacing={0}>
          ...
          <Box fontSize="xs" color="gray.500">#{row.playerNumber}</Box>
        </VStack>
      ),
    },
    {
      ...
      render: (row) => (
        <VStack align="start" spacing={0}>
          ...
          <Text fontSize="xs" color="gray.500">{row.matchDate}</Text>
        </VStack>
      ),
    },
    {
  ...
  const getFilterValueOptions = () => {
    if (filterType === 'player') {
      return [{ value: 'all', label: 'All Players' }, ...uniquePlayers.map(p => ({ value: p, label: p }))];
    } else if (filterType === 'match') {
      return [{ value: 'all', label: 'All Matches' }, ...uniqueMatches.map(m => ({ value: m, label: m }))];
    }
    return [];
  };
  ...
  return (
    <Layout pageTitle="Statistics Management" pageSubtitle="Manage player performance statistics">
      ...
      <Modal isOpen={isAddOpen} onClose={onAddClose} size="lg">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Add Player Statistics</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <VStack spacing={4}>
              ...
              <FormControl isRequired>
                ...
                <Input
                  ...
                  placeholder="e.g., Academy U17 vs Riverside FC"
                />
              </FormControl>
      ...
      <Modal isOpen={isEditOpen} onClose={onEditClose} size="lg">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Edit Statistics</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <VStack spacing={4}>
              <FormControl isRequired>
      ...
      <Modal isOpen={isDeleteOpen} onClose={onDeleteClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Delete Statistics</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            Are you sure you want to delete these statistics? This action cannot be undone.
          </ModalBody>
          <ModalFooter>
